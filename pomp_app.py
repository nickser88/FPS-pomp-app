...
with col1:
    ...
    actueel_rpm = st.number_input("Speed (RPM)", value=2900, step=50)
...
def generate_pdf(all_curves, meta):
    ...
    def draw_header(canvas, document):
        ...
        # Date / page line bottom-right of band
    ...
    def draw_footer(canvas, document):
        ...
        canvas.drawString(MARGIN, 5*mm,
            f"Generated {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}  ·  FPS Pump Test App  ·  Confidential")
    ...
    for curve in all_curves:
        ...
        mt.setStyle(TableStyle([
            ('BACKGROUND',    (0, 0), (-1, 0),  colors.HexColor('#990000')),  # ← comma was missing here
        ...
        story.append(KeepTogether([mt, Spacer(1, 4*mm)]))

    # ── 4. DECLARATION / SIGN-OFF ────────────────────────────────────────────
    story += section_header("4.  Declaration")
